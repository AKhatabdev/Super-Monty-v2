 ...
 *
 */
public class IntroScreen extends SuperMontyScreen {
	private final Animation<TextureRegion> animation = GifDecoder.loadGIFAnimation(Animation.PlayMode.LOOP, Gdx.files.internal("data/intro.gif").read());
	SpriteBatch batch;
	float time = 0;
    float elapsed;
	...
	public void render (float delta) {
		...
		if (time > 2) {
			if (Gdx.input.isKeyPressed(Keys.ANY_KEY) || Gdx.input.justTouched()) {
				...
				game.setScreen(new ControlIntroScreen(game));
			}
		}
	}
